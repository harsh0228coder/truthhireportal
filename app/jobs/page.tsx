...
const getDepartment = (title: string): string => {
  ...
  if (
    ...
    t.includes("account man") ||
    t.includes("client growth") ||
    t.includes("revenue")
  )
    return "Sales";
...
function JobsPageContent() {
      ...
      try {
        ...
        if (token) {
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/users/me/saved-ids`,
            {
  ...
  const departmentCounts = jobs.reduce(
    (acc, job) => {
  ...
  const getModalData = () => {
    const source =
      activeModal === "location" ? sortedLocations : sortedDepartments;
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length > 1) {
      ...
      jobs.forEach((job) => {
        if (job.title?.toLowerCase().includes(lowerValue))
          uniqueSuggestions.add(job.title);
        if (job.company_name?.toLowerCase().includes(lowerValue))
          uniqueSuggestions.add(job.company_name);
      });
  ...
  useEffect(() => {
    ...
    if (selectedExpLevels.length > 0) {
      result = result.filter((job) => {
        ...
        return selectedExpLevels.some((filterLevel) => {
          const normalizedFilter = filterLevel
            .toLowerCase()
            .replace(/[^a-z]/g, "");
          return (
            jobLevel.includes(normalizedFilter) ||
            normalizedFilter.includes(jobLevel)
          );
        });
      });
    }
    ...
    if (minSalary > 0) {
      result = result.filter(
        (job) => (job.salary_max || 0) >= minSalary * 100000,
      );
    }
  ...
  const handleSaveJob = async (jobIdInput: number | string) => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/jobs/${jobId}/${endpoint}`,
        {
          method: method,
          headers: {
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {[
            ...
            >
              <div
                className={`w-4 h-4 rounded-full border flex items-center justify-center transition-all ${datePosted === opt.value ? "border-electric" : "border-white/20 group-hover:border-white/40"}`}
              >
              ...
              />
              <span
                className={`text-sm ${datePosted === opt.value ? "text-white font-medium" : "text-gray-400 group-hover:text-gray-300"}`}
              >
                {opt.label}
              </span>
            </label>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2">
            <DollarSign size={12} /> Min Salary (LPA)
          </h4>
          <span className="text-xs text-electric font-bold">
        ...
        <input
          ...
          className="w-full h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-electric hover:accent-blue-400 focus:outline-none"
        />
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {sortedDepartments.slice(0, 5).map((dept) => (
            ...
            >
              <div
                className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${selectedDepartments.includes(dept) ? "bg-electric border-electric" : "bg-[#1a1a1a] border-white/20 group-hover:border-white/40"}`}
              >
              ...
              />
              <span
                className={`text-sm ${selectedDepartments.includes(dept) ? "text-white" : "text-gray-400 group-hover:text-gray-300"}`}
              >
                {dept}{" "}
          ...
          {sortedDepartments.length > 5 && (
            <button
              ...
              className="text-xs font-bold text-electric hover:underline mt-2 block"
            >
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {sortedLocations.slice(0, 5).map((loc) => (
            ...
            >
              <div
                className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${selectedLocations.includes(loc) ? "bg-electric border-electric" : "bg-[#1a1a1a] border-white/20 group-hover:border-white/40"}`}
              >
              ...
              <input
                ...
                onChange={() =>
                  toggleSelection(selectedLocations, setSelectedLocations, loc)
                }
              />
              <span
                className={`text-sm ${selectedLocations.includes(loc) ? "text-white" : "text-gray-400 group-hover:text-gray-300"}`}
              >
                {loc}{" "}
                <span className="text-xs text-gray-600">
                  ({locationCounts[loc]})
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {jobTypeOptions.map((type) => (
            ...
            >
              <div
                className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${selectedJobTypes.includes(type) ? "bg-electric border-electric" : "bg-[#1a1a1a] border-white/20 group-hover:border-white/40"}`}
              >
              ...
              <input
                ...
                onChange={() =>
                  toggleSelection(selectedJobTypes, setSelectedJobTypes, type)
                }
              />
              <span
                className={`text-sm ${selectedJobTypes.includes(type) ? "text-white" : "text-gray-400 group-hover:text-gray-300"}`}
              >
                {type}
              </span>
            </label>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          {experienceOptions.map((level) => (
            ...
            >
              <div
                className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${selectedExpLevels.includes(level) ? "bg-electric border-electric" : "bg-[#1a1a1a] border-white/20 group-hover:border-white/40"}`}
              >
                {selectedExpLevels.includes(level) && (
                  <Check size={10} className="text-white" />
                )}
              </div>
              <input
              ...
              />
              <span
                className={`text-sm ${selectedExpLevels.includes(level) ? "text-white" : "text-gray-400 group-hover:text-gray-300"}`}
              >
                {level}
              </span>
            </label>
      ...
      <div className="relative pt-20 pb-12 overflow-visible border-b border-white/5 z-40">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="bg-[#111]/80 backdrop-blur-xl p-2 rounded-2xl border border-white/10 shadow-2xl shadow-black/50 flex flex-col md:flex-row items-stretch gap-3 md:gap-10 max-w-4xl mx-auto relative z-50">
            ...
            <div
              className="flex-1 w-full flex items-center bg-transparent px-4 h-12 md:h-14 group border-b border-white/10 md:border-b-0 relative"
              ...
              {searchQuery && (
                <button
                  onClick={() => {
                    ...
                    // ✅ FIX: Remove the query param from the URL immediately
                  ...
                  <div className="py-2">
                    ...
                    {suggestions.map((suggestion, index) => (
                      ...
                      >
                        <Search
                          size={14}
                          className="text-gray-600 group-hover:text-electric"
                        />
                        {suggestion}
                      </button>
                    ))}
            ...
            <div
              className="flex-1 w-full relative h-12 md:h-14 group"
              ref={locationRef}
            >
              <button
                onClick={() =>
                ...
                className="w-full h-full flex items-center bg-transparent px-4 focus:outline-none"
              >
                <MapPin
                  className={`h-5 w-5 mr-3 transition-colors ${isLocationDropdownOpen ? "text-electric" : "text-gray-500 group-hover:text-gray-300"}`}
                />
                <span
                  className={`text-base truncate ${locationFilter ? "text-white" : "text-gray-500"}`}
                >
                  {locationFilter || "Any Location"}
                </span>
                <ChevronDown
                  className={`h-4 w-4 text-gray-500 absolute right-4 transition-transform duration-200 ${isLocationDropdownOpen ? "rotate-180 text-electric" : ""}`}
                />
              </button>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-[60] animate-in fade-in zoom-in-95 duration-150">
                  <div className="max-h-60 overflow-y-auto custom-scrollbar">
                    <button
                      ...
                      className="w-full text-left px-4 py-3 text-sm text-gray-400 hover:bg-white/5 hover:text-white border-b border-white/5 flex items-center justify-between"
                    >
                      Any Location{" "}
                      {locationFilter === "" && (
            ...
            <button className="inline-flex h-12 px-8 mt-1 bg-white text-black hover:bg-gray-200 rounded-lg font-semibold text-base items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(255,255,255,0.3)] hover:shadow-blue-500/30 hover:scale-105">
              Search
            </button>
          </div>
        </div>
            ...
            <div className="bg-[#111] border border-white/10 rounded-xl overflow-hidden shadow-2xl h-fit">
              <div className="flex items-center justify-between p-5 border-b border-white/10 bg-[#161616]">
                ...
                <button
                  ...
                  className="text-[11px] font-bold text-gray-500 hover:text-white uppercase tracking-wider transition-colors"
                >
                  Reset
                </button>
              </div>
          ...
          <main className="flex-1 relative z-0">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4 relative z-30">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="lg:hidden flex items-center gap-2 bg-[#111] border border-white/10 text-sm text-white px-4 py-2 rounded-xl hover:bg-white/5 transition-colors"
                >
                ...
                <div className="relative group z-30" ref={sortRef}>
                  <button
                    ...
                    className="flex items-center gap-2 bg-[#111] border border-white/10 text-sm text-white pl-4 pr-3 py-2 rounded-xl hover:border-white/30 transition-colors focus:outline-none"
                  >
            ...
            {filteredJobs.length > 0 ? (
              ...
              <div className="text-center py-24 bg-[#111]/50 rounded-2xl border border-white/5 border-dashed">
                ...
                <p className="text-gray-500 text-sm mt-2 max-w-xs mx-auto mb-8">
                  We couldn't find any jobs matching your filters. Try widening
                  your search or clearing filters.
                </p>
                <button
                  onClick={clearAllFilters}
                  className="text-electric font-bold hover:text-white transition-colors flex items-center gap-2 mx-auto"
                >
            ...
            {!loading && filteredJobs.length > jobsPerPage && (
              <div className="mt-12 flex justify-center items-center gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-lg text-sm font-medium text-gray-400 hover:text-white hover:bg-white/5 disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
                >
                  Previous
                </button>
                    ...
                    if (
                      ...
                      return (
                        <button
                          ...
                          className={`w-9 h-9 rounded-md text-sm font-bold transition-all ${currentPage === page ? "bg-electric text-white shadow-lg shadow-blue-900/50" : "text-gray-400 hover:text-white hover:bg-white/5"}`}
                        >
                          {page}
                        </button>
                      );
        ...
        <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-[#111] border border-white/10 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[80vh] overflow-hidden">
            ...
            <div className="p-4 border-b border-white/5">
              <div className="flex items-center bg-[#0a0a0a] border border-white/10 rounded-lg px-4 h-12 focus-within:border-electric transition-colors">
                ...
                <input
                  ...
                  className="w-full bg-transparent border-none focus:ring-0 focus:outline-none text-white ml-3 text-sm"
                />
              </div>
            </div>
                ...
                {getModalData().length > 0 ? (
                  getModalData().map((item) => (
                    ...
                    >
                      ...
                      />
                      <span
                        className={`text-sm ${(activeModal === "location" ? selectedLocations.includes(item) : selectedDepartments.includes(item)) ? "text-white font-medium" : "text-gray-400 group-hover:text-gray-300"}`}
                      >
                        {item}{" "}
                        <span className="text-xs text-gray-600">
                          (
            ...
            <div className="p-4 border-t border-white/10 flex justify-end gap-3 bg-[#161616]">
              <button
                onClick={() => setActiveModal("none")}
                className="px-5 py-2.5 rounded-lg border border-white/10 text-gray-400 hover:text-white hover:bg-white/5 text-sm font-bold transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={() => setActiveModal("none")}
                className="px-6 py-2.5 rounded-lg bg-electric hover:bg-blue-600 text-white text-sm font-bold transition-colors shadow-lg shadow-blue-900/20"
              >
                Apply
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-[85%] max-w-[320px] bg-[#111] border-l border-white/10 shadow-2xl flex flex-col animate-slideInRight">
            ...
            <div className="p-5 border-t border-white/10 bg-[#161616] flex gap-3">
              <button
                onClick={clearAllFilters}
                className="flex-1 py-3 rounded-xl border border-white/10 text-gray-400 font-bold hover:text-white"
              >
                Reset
              </button>
              <button
                onClick={() => setIsMobileFilterOpen(false)}
                className="flex-1 py-3 rounded-xl bg-electric text-white font-bold hover:bg-blue-600 shadow-lg shadow-blue-900/20"
              >
                Apply
              </button>
            </div>
