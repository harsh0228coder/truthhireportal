...
import {
  User, Briefcase, Building2, Zap, Github,
  ArrowLeft, Save, Plus, Trash2, Camera,
  Loader2, X, GraduationCap, Calendar,
  Code, ChevronDown, Check, Square, CheckSquare, Search, Wallet
} from "lucide-react";
...
const POPULAR_SKILLS = [
  ...
  "HTML", "CSS", "Tailwind CSS", "Bootstrap", "Sass", "GraphQL", "REST API", "SQL", "NoSQL", "MongoDB", "PostgreSQL",
  ...
  "Figma", "Adobe XD", "UI/UX Design", "Product Management", "Agile", "Scrum", "Jira", "Project Management",
  "Communication", "Leadership", "Teamwork", "Problem Solving", "Critical Thinking", "Time Management",
  "Sales", "Marketing", "SEO", "Content Writing", "Social Media Marketing", "Email Marketing",
  "Finance", "Accounting", "Excel", "Power BI", "Tableau", "Blockchain", "Cybersecurity", "Networking"
];
...
const getErrorMessage = (data: any) => {
    try {
        ...
        if (Array.isArray(data.detail)) return data.detail[0]?.msg || "Invalid input data";
...
const InputGroup = ({ label, value, onChange, placeholder, type = "text", className = "", error, required }: any) => (
...
const CustomSelect = ({ label, value, onChange, options, placeholder = "Select", error, disabled = false }: any) => {
...
const TextAreaGroup = ({ label, value, onChange, placeholder, rows = 4, error }: any) => (
  <div className="mb-5">
    <label className="block text-xs text-gray-400 font-bold uppercase tracking-wider mb-2 flex justify-between">
        <span>{label}</span>
        {error && <span className="text-red-400 normal-case">{error}</span>}
...
// --- MODAL ---
const EditModal = ({ isOpen, onClose, title, children, onSave, saving }: any) => {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-[#111] border border-white/10 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 border-t border-white/5 bg-[#151515] rounded-b-2xl flex justify-end gap-3">
                    <button onClick={onClose} className="px-6 py-2.5 rounded-xl border border-white/10 text-gray-300 hover:bg-white/5 font-medium transition">
                        Cancel
                    </button>
                    <button 
                        onClick={onSave} 
...
export default function EditProfilePage() {
    ...
    const fetchUser = async () => {
      ...
      try {
        ...
        if (eduList.length === 0 && (data.college || data.degree)) {
            eduList = [{ school: data.college, degree: data.degree, year: data.batch_year, field: "" }];
        }
  ...
  const handleSaveProfile = async (payload: any) => {
    ...
    ['github_url', 'linkedin_url', 'portfolio_url'].forEach(key => {
        if (sanitizedPayload[key] && sanitizedPayload[key].toString().trim() !== "" && !sanitizedPayload[key].toString().startsWith('http')) {
  ...
  const saveProject = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (isEdit) {
            newProjects = newProjects.map(p => p.id === savedProject.id ? savedProject : p);
        } else {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-blue-500/30">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="flex items-center gap-4 mb-10">
            ...
            <div>
                ...
                <p className="text-gray-400 text-sm">Update your personal details and resume.</p>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8">
            ...
            <div className="md:col-span-3">
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <InputGroup label="Full Name" value={user.name} onChange={(v: string) => setUser({...user, name: v})} required error={errors.name} />
                                <InputGroup label="Phone" value={user.phone} onChange={(v: string) => setUser({...user, phone: v})} required error={errors.phone} />
                            </div>
                            <InputGroup label="Headline" value={user.headline} onChange={(v: string) => setUser({...user, headline: v})} />
                            <InputGroup label="Location" value={user.location} onChange={(v: string) => setUser({...user, location: v})} required error={errors.location} />
                        </div>

                        <div className="space-y-4 pt-4">
                            <h3 className="text-lg font-bold text-white border-b border-white/5 pb-2 flex items-center gap-2"><Wallet size={18} className="text-green-500"/> Career & Compensation</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <InputGroup label="Total Experience (Years)" value={user.total_experience} onChange={(v: string) => setUser({...user, total_experience: v})} type="number"/>
                                <CustomSelect label="Notice Period" value={user.notice_period} onChange={(v: string) => setUser({...user, notice_period: v})} options={["Immediate", "15 Days", "1 Month", "2 Months", "3 Months", "Serving Notice Period"]} />
                                <InputGroup label="Current CTC" value={user.current_salary} onChange={(v: string) => setUser({...user, current_salary: v})} />
                                <InputGroup label="Expected CTC" value={user.expected_salary} onChange={(v: string) => setUser({...user, expected_salary: v})} />
                            </div>
                        </div>

                        <TextAreaGroup label="About" value={user.bio} onChange={(v: string) => setUser({...user, bio: v})} />
                        
                        <div className="flex justify-end pt-4">
                            <button onClick={saveGeneral} disabled={saving} className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-3 rounded-xl font-bold transition disabled:opacity-50 flex items-center gap-2">
                ...
                {activeTab === "experience" && (
                    <div className="space-y-6">
                        <div className="flex justify-between items-center border-b border-white/5 pb-4">
                            ...
                            <button onClick={() => { setEditingItem({ role: "", company: "", start_date: "", end_date: "", description: "", is_current: false }); setErrors({}); setModalType("experience"); }} className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2 rounded-lg text-sm text-white transition"><Plus size={16} /> Add New</button>
                        </div>
                        {experiences.length === 0 ? (
                            ...
                            <div className="space-y-4">
                                {experiences.map((exp, idx) => (
                                    <div key={idx} className="bg-[#111] border border-white/5 p-5 rounded-xl flex justify-between items-start group hover:border-white/20 transition-all">
                                        <div>
                                            {/* --- FIX: Display both keys to handle legacy data --- */}
                                            <h4 className="font-bold text-white text-lg">{exp.role || exp.job_title}</h4>
                                            <p className="text-blue-400 text-sm mb-1">{exp.company}</p>
                                            <p className="text-xs text-gray-500 mb-3">{exp.start_date} - {exp.end_date}</p>
                                            <p className="text-sm text-gray-400 line-clamp-2">{exp.description}</p>
                                        </div>
                                        <div className="flex gap-2">
                                            {/* --- FIX: Map job_title to role when editing --- */}
                                            <button onClick={() => { setEditingItem({ ...exp, role: exp.role || exp.job_title, index: idx, is_current: exp.end_date === 'Present' }); setErrors({}); setModalType("experience"); }} className="p-2 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white"><Zap size={16} /></button>
                                            <button onClick={() => deleteExperience(idx)} className="p-2 hover:bg-red-500/10 rounded-lg text-gray-400 hover:text-red-400"><Trash2 size={16} /></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {activeTab === "education" && (
                    <div className="space-y-6">
                        <div className="flex justify-between items-center border-b border-white/5 pb-4">
                            ...
                            <button onClick={() => { setEditingItem({ school: "", degree: "", field: "", year: "", grade: "" }); setErrors({}); setModalType("education"); }} className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2 rounded-lg text-sm text-white transition"><Plus size={16} /> Add New</button>
                        </div>
                        {educations.length === 0 ? (
                            <div className="text-center py-12 border border-dashed border-white/10 rounded-xl bg-white/[0.02]">
                            ...
                            <div className="space-y-4">
                                {educations.map((edu, idx) => (
                                    <div key={idx} className="bg-[#111] border border-white/5 p-5 rounded-xl flex justify-between items-start group hover:border-white/20 transition-all">
                                        <div>
                                            <h4 className="font-bold text-white text-lg">{edu.school}</h4>
                                            <p className="text-blue-400 text-sm mb-1">{edu.degree} {edu.field ? `• ${edu.field}` : ""}</p>
                                            <p className="text-xs text-gray-500">{edu.year} {edu.grade && `• Grade: ${edu.grade}`}</p>
                                        </div>
                                        <div className="flex gap-2">
                                            <button onClick={() => { setEditingItem({ ...edu, index: idx }); setErrors({}); setModalType("education"); }} className="p-2 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white"><Zap size={16} /></button>
                                            <button onClick={() => deleteEducation(idx)} className="p-2 hover:bg-red-500/10 rounded-lg text-gray-400 hover:text-red-400"><Trash2 size={16} /></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {activeTab === "projects" && (
                    <div className="space-y-6">
                        <div className="flex justify-between items-center border-b border-white/5 pb-4">
                            ...
                            <button onClick={() => { setEditingItem({ title: "", description: "", tech_stack: "", live_link: "", github_link: "" }); setErrors({}); setModalType("project"); }} className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2 rounded-lg text-sm text-white transition"><Plus size={16} /> Add New</button>
                        </div>
                        {projects.length === 0 ? (
                            <div className="text-center py-12 border border-dashed border-white/10 rounded-xl bg-white/[0.02]">
                                    ...
                                    <div key={idx} className="bg-[#111] border border-white/5 p-5 rounded-xl flex justify-between items-start group hover:border-white/20 transition-all">
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-2 my-2">
                                                {proj.tech_stack?.split(',').map((t: string, i: number) => (
                                                    <span key={i} className="text-[10px] bg-blue-500/10 text-blue-300 px-2 py-1 rounded border border-blue-500/20">{t.trim()}</span>
                                                ))}
                                            </div>
                                            <p className="text-sm text-gray-400 line-clamp-2">{proj.description}</p>
                                        </div>
                                        <div className="flex gap-2">
                                            <button onClick={() => { setEditingItem(proj); setErrors({}); setModalType("project"); }} className="p-2 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white"><Zap size={16} /></button>
                                            <button onClick={() => deleteProject(proj.id)} className="p-2 hover:bg-red-500/10 rounded-lg text-gray-400 hover:text-red-400"><Trash2 size={16} /></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="bg-[#111] border border-white/10 p-6 rounded-xl">
                            <SkillSelector 
                                ...
                                onChange={(newSkills: string[]) => setUser({...user, skills: newSkills})} 
                            />
                        </div>
                        <div className="flex justify-end">
                            <button onClick={() => handleSaveProfile({ skills: user.skills })} disabled={saving} className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-3 rounded-xl font-bold transition disabled:opacity-50 flex items-center gap-2">
                                {saving && <Loader2 size={18} className="animate-spin" />} Save Skills
                            </button>
                        </div>
                    </div>
                )}
                ...
                {activeTab === "socials" && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                        <h3 className="text-lg font-bold text-white border-b border-white/5 pb-2">Social Links</h3>
                        <div className="bg-[#111] border border-white/10 p-6 rounded-xl space-y-4">
                            <div className="flex items-center gap-4"><Github className="text-white shrink-0" size={24} /><div className="flex-1"><InputGroup label="GitHub URL" value={user.github_url} onChange={(v: string) => setUser({...user, github_url: v})} className="mb-0" /></div></div>
                            <div className="flex items-center gap-4"><div className="w-6 flex justify-center shrink-0"><span className="font-bold text-blue-500 text-xl">in</span></div><div className="flex-1"><InputGroup label="LinkedIn URL" value={user.linkedin_url} onChange={(v: string) => setUser({...user, linkedin_url: v})} className="mb-0" /></div></div>
                            <div className="flex items-center gap-4"><div className="w-6 flex justify-center shrink-0"><span className="font-bold text-purple-500 text-xl">W</span></div><div className="flex-1"><InputGroup label="Portfolio / Website" value={user.portfolio_url} onChange={(v: string) => setUser({...user, portfolio_url: v})} className="mb-0" /></div></div>
                        </div>
                        <div className="flex justify-end">
                            <button onClick={() => handleSaveProfile({ github_url: user.github_url, linkedin_url: user.linkedin_url, portfolio_url: user.portfolio_url })} disabled={saving} className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-3 rounded-xl font-bold transition disabled:opacity-50 flex items-center gap-2">
                                {saving && <Loader2 size={18} className="animate-spin" />} Save Socials
                            </button>
                        </div>
                    </div>
                )}
      ...
      {modalType === "experience" && (
          <EditModal isOpen={true} onClose={() => setModalType(null)} title={editingItem.index !== undefined ? "Edit Experience" : "Add Experience"} onSave={saveExperience} saving={saving}>
              {/* AUTO-SUGGEST FOR JOB TITLE */}
              <AutoSuggestionInput 
                  label="Job Title" 
                  value={editingItem.role} 
                  onChange={(v:string) => setEditingItem({...editingItem, role: v})} 
                  options={POPULAR_JOB_TITLES}
                  placeholder="e.g. Senior Software Engineer"
                  required 
                  error={errors.role} 
              />
              <InputGroup label="Company" value={editingItem.company} onChange={(v:string) => setEditingItem({...editingItem, company: v})} required error={errors.company} />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <MonthYearPicker label="Start Date" value={editingItem.start_date} onChange={(v:string) => setEditingItem({...editingItem, start_date: v})} error={errors.start_date} />
                  <MonthYearPicker label="End Date" value={editingItem.end_date} onChange={(v:string) => setEditingItem({...editingItem, end_date: v})} disabled={editingItem.is_current} error={errors.end_date} />
              </div>
              <div className="flex items-center gap-2 mb-5">
                  <button onClick={() => setEditingItem({...editingItem, is_current: !editingItem.is_current})} className={`flex items-center gap-2 text-sm ${editingItem.is_current ? 'text-blue-400' : 'text-gray-400'}`}>
                      {editingItem.is_current ? <CheckSquare size={18} /> : <Square size={18} />}
                      Currently working here
                  </button>
              </div>
              <TextAreaGroup label="Description" value={editingItem.description} onChange={(v:string) => setEditingItem({...editingItem, description: v})} />
          </EditModal>
      )}

      {modalType === "education" && (
          <EditModal isOpen={true} onClose={() => setModalType(null)} title={editingItem.index !== undefined ? "Edit Education" : "Add Education"} onSave={saveEducation} saving={saving}>
              <InputGroup label="School / University" value={editingItem.school} onChange={(v:string) => setEditingItem({...editingItem, school: v})} required error={errors.school} />
              
              {/* AUTO-SUGGEST FOR DEGREE */}
              <AutoSuggestionInput 
                  label="Degree" 
                  value={editingItem.degree} 
                  onChange={(v:string) => setEditingItem({...editingItem, degree: v})} 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <AutoSuggestionInput 
                      ...
                      onChange={(v:string) => setEditingItem({...editingItem, field: v})} 
                      options={POPULAR_FIELDS}
                      placeholder="e.g. Computer Science"
                  />
                  <CustomSelect label="Graduation Year" value={editingItem.year} onChange={(v:string) => setEditingItem({...editingItem, year: v})} options={years} error={errors.year} />
              </div>
              <InputGroup label="Grade / CGPA" value={editingItem.grade} onChange={(v:string) => setEditingItem({...editingItem, grade: v})} />
          </EditModal>
      )}

      {modalType === "project" && (
          <EditModal isOpen={true} onClose={() => setModalType(null)} title={editingItem.id ? "Edit Project" : "Add Project"} onSave={saveProject} saving={saving}>
              <InputGroup label="Project Title" value={editingItem.title} onChange={(v:string) => setEditingItem({...editingItem, title: v})} required error={errors.title} />
              <InputGroup label="Tech Stack (Comma Separated)" value={editingItem.tech_stack} onChange={(v:string) => setEditingItem({...editingItem, tech_stack: v})} />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <InputGroup label="Live URL" value={editingItem.live_link} onChange={(v:string) => setEditingItem({...editingItem, live_link: v})} />
                  <InputGroup label="GitHub URL" value={editingItem.github_link} onChange={(v:string) => setEditingItem({...editingItem, github_link: v})} />
              </div>
              <TextAreaGroup label="Description" value={editingItem.description} onChange={(v:string) => setEditingItem({...editingItem, description: v})} required error={errors.description} />
          </EditModal>
      )}
