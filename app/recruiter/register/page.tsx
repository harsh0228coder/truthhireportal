...
export default function RecruiterRegister() {
  ...
  useEffect(() => {
    const email = formData.official_email.toLowerCase();
    const publicDomains = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'icloud.com', 'rediffmail.com', 'protonmail.com'];
    
    if (email.includes('@')) {
        const domain = email.split('@')[1];
        setIsPublicEmail(publicDomains.includes(domain));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:8000/recruiters/register", {
        method: "POST",
  ...
  const handleVerifyOtp = async () => {
    ...
    setVerifying(true);
    try {
      const response = await fetch('http://localhost:8000/recruiters/verify-signup-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#09090b] p-4 font-sans text-white relative overflow-hidden">
      ...
      {/* --- MAIN CARD --- */}
      <div className="w-full max-w-[26rem] bg-[#131316] border border-[#27272a] rounded-xl p-6 shadow-[0_15px_30px_-10px_rgba(0,0,0,0.6)] relative z-10 flex flex-col gap-5">
        ...
        <div className="text-center">
             ...
             <h1 className="text-[16px] font-semibold text-white">Recruiter Access</h1>
             <p className="text-[#a1a1aa] text-[14px] mt-0.5">Post verified jobs and find top talent.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
                <label className="text-[13px] font-medium text-[#f6f6f7] block ml-0.5">Full Name</label>
                <div className="relative">
                    <User className="absolute left-3 top-1/2 -translate-y-1/2 text-[#71717a] w-3.5 h-3.5" />
                    <input
                    name="name"
                    type="text"
                    value={formData.name}
            ...
            <div className="space-y-1">
                <label className="text-[13px] font-medium text-[#f6f6f7] block ml-0.5">Company</label>
                <div className="relative">
                    <Building2 className="absolute left-3 top-1/2 -translate-y-1/2 text-[#71717a] w-3.5 h-3.5" />
                    <input
                    name="company_name"
                    type="text"
          ...
          <div className="space-y-1">
            <label className="text-[13px] font-medium text-[#f6f6f7] block ml-0.5">
                LinkedIn URL
                {isPublicEmail ? <span className="text-red-400 ml-1">*</span> : <span className="text-[#52525b] font-normal ml-1">(Optional)</span>}
            </label>
            <div className="relative">
                <Linkedin className="absolute left-3 top-1/2 -translate-y-1/2 text-[#71717a] w-3.5 h-3.5" />
                <input
                name="linkedin_url"
                type="url"
                value={formData.linkedin_url}
                onChange={handleChange}
                required={isPublicEmail}
                className="w-full bg-[#09090b] border border-[#27272a] focus:border-blue-600 focus:ring-1 focus:ring-blue-600/20 rounded-lg pl-9 pr-3 h-9 text-[13px] text-white placeholder-[#3f3f46] outline-none transition-all"
                placeholder="linkedin.com/in/..."
                />
            </div>
            {isPublicEmail && (
                <div className="mt-1 flex items-start gap-1.5 bg-yellow-500/10 border border-yellow-500/20 rounded-md p-2">
                  <AlertCircle className="w-3 h-3 text-yellow-500 shrink-0 mt-0.5" />
                  <p className="text-[10px] text-yellow-200/80 leading-tight">
                    LinkedIn required for verification when using public email domains (Gmail, Yahoo, etc).
                  </p>
                </div>
            )}
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[13px] font-medium text-[#f6f6f7] block ml-0.5">Password</label>
            <div className="relative">
          ...
          >
            {loading ? <Loader2 className="w-3.5 h-3.5 animate-spin mx-auto" /> : 'Create Recruiter Account'}
          </button>
        </form>
        ...
        <div className="text-center pt-2">
            <p className="text-[12px] text-[#71717a]">
                ...
                <Link href="/recruiter/login" className="text-blue-500 hover:text-blue-400 font-medium hover:underline">
                    Sign in
                </Link>
            </p>
          ...
          <div className="bg-[#131316] border border-[#27272a] rounded-xl p-6 w-full max-w-[340px] shadow-2xl relative">
            ...
            <div className="text-center mb-5">
              ...
              <p className="text-[13px] text-[#a1a1aa] leading-relaxed">
                We sent a code to <span className="text-white font-medium">{formData.official_email}</span>
              </p>
            </div>
            ...
            <button
              ...
              disabled={verifying}
              className="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium h-9 rounded-lg transition-all text-sm disabled:opacity-50"
            >
              {verifying ? <Loader2 className="w-3.5 h-3.5 animate-spin mx-auto" /> : 'Verify Code'}
            </button>

            <p className="text-center text-[13px] text-[#71717a] mt-4">
      ...
      {showPendingModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-[#131316] border border-yellow-500/20 rounded-xl p-6 w-full max-w-[30rem] shadow-2xl relative text-center">
            ...
            <h2 className="text-[15px] font-semibold text-white mb-2">Verification In Progress</h2>
            <p className="text-[13px] text-[#a1a1aa] mb-4 leading-relaxed">
                Since you used a public email domain, our team needs to manually verify your LinkedIn profile to ensure platform safety.
            </p>
            
            <div className="bg-white/5 border border-white/5 rounded-lg p-3 mb-4 text-left">
                <p className="text-[14px] text-gray-300 font-semibold mb-1.5">Next Steps:</p>
                <ul className="text-[13px] text-gray-500 list-disc list-inside space-y-1">
                    <li>Review takes 2-4 hours.</li>
                    <li>You'll get an email once approved.</li>
                    <li>Job posting is restricted until then.</li>
                </ul>
            </div>

            <button 
                onClick={() => router.push('/recruiter/login')}
                className="w-full bg-[#27272a] hover:bg-[#3f3f46] text-white font-semibold h-9 rounded-lg transition-all text-sm border border-[#3f3f46]"
            >
                Go to Login
            </button>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-[60] p-4 animate-in fade-in duration-300">
          <div className="bg-[#131316] border border-[#27272a] rounded-2xl p-8 w-full max-w-[340px] shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative flex flex-col items-center text-center animate-in zoom-in-95 duration-300">
            ...
            <p className="text-[#a1a1aa] text-sm mb-6 leading-relaxed">
              Your company identity has been verified. <br/> Redirecting to dashboard...
            </p>
