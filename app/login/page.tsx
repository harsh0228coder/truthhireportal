...
export default function LoginPage() {
  ...
  const handleForgotRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResetError(d.detail || "Email not found");
      }
    } catch (err) {
  ...
  const handleVerifyResetOtp = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/users/verify-reset-otp`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        const d = await res.json();
        setResetError(d.detail || "Invalid Code");
        setResetOtp(["", "", "", "", "", ""]);
      }
    } catch (err) {
  ...
  const handleFinalReset = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 6) {
      setResetError("Password too short");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          ...
          // Just close modal, user is already on login page to log in
        }, 2000);
      } else {
        const d = await res.json();
        setResetError(d.detail || "Failed to reset");
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#09090b] p-4 font-sans text-white relative overflow-hidden">
      ...
      <div className="w-full max-w-[25rem] bg-[#131316] border border-[#27272a] rounded-xl p-6 shadow-[0_15px_30px_-10px_rgba(0,0,0,0.6)] relative z-10 flex flex-col gap-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[14px] text-[#71717a] font-medium lowercase">
            or
          </span>
          <div className="h-px bg-[#27272a] flex-1"></div>
        </div>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-3">
          ...
          <div className="space-y-1">
            <label className="text-[14px] font-semibold text-[#f6f6f7] block ml-0.5">
              Email address
            </label>
            <input
              type="email"
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center ml-0.5">
              <label className="text-[14px] font-semibold text-[#f6f6f7]">
                Password
              </label>
              <button
                type="button"
      ...
      {/* ========================================== */}
      {showForgotModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[55] p-4 animate-in fade-in duration-200">
          <div className="bg-[#131316] border border-[#27272a] rounded-xl p-6 w-full max-w-[360px] shadow-2xl relative">
            {/* Close Button */}
            <button
              onClick={() => setShowForgotModal(false)}
              className="absolute top-3 right-3 text-[#71717a] hover:text-white transition-colors"
            >
              ...
              <div className="animate-in slide-in-from-right-4 duration-300">
                ...
                <form onSubmit={handleForgotRequest}>
                  <input
                    ...
                    className="w-full bg-[#09090b] border border-[#27272a] focus:border-blue-600 rounded-lg px-3 h-10 text-[13px] text-white outline-none mb-4"
                  ...
                  <button
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium h-9 rounded-lg transition-all text-xs flex items-center justify-center"
                  >
                    {resetLoading ? (
            ...
            {resetStep === 2 && (
              <div className="animate-in slide-in-from-right-4 duration-300">
                <button
                  ...
                  className="absolute top-3 left-3 text-[#71717a] hover:text-white transition-colors"
                >
                ...
                <div className="flex gap-2 justify-center mb-6">
                  {resetOtp.map((digit, index) => (
                    <input
                      ...
                      className="otp-input w-9 h-11 text-center text-lg font-bold bg-[#09090b] border border-[#27272a] focus:border-blue-600 rounded-lg text-white outline-none"
                      placeholder="-"
                    />
                  ))}
                </div>
            ...
            {resetStep === 3 && (
              <div className="animate-in slide-in-from-right-4 duration-300">
                ...
                <form onSubmit={handleFinalReset} className="space-y-3">
                  <div className="space-y-1">
                    <label className="text-[11px] text-gray-400 ml-1">
                      New Password
                    </label>
                    <input
                      type="password"
                      ...
                      className="w-full bg-[#09090b] border border-[#27272a] focus:border-blue-600 rounded-lg px-3 h-9 text-[13px] text-white outline-none"
                      required
                    />
                  </div>
                  <div className="space-y-1">
                  ...
                  <button
                    type="submit"
                    disabled={resetLoading}
                    className="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium h-9 rounded-lg transition-all text-xs flex items-center justify-center mt-2"
                  >
                    {resetLoading ? (
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    ) : (
          ...
          <div className="bg-[#131316] border border-[#27272a] rounded-xl p-6 w-full max-w-[340px] shadow-2xl relative">
            ...
            <div className="flex gap-2 justify-center mb-6">
              {otp.map((digit, index) => (
                <input
                  key={index}
                  className="otp-input w-9 h-11 text-center text-lg font-bold bg-[#09090b] border border-[#27272a] focus:border-blue-600 rounded-lg text-white outline-none transition-all placeholder-[#3f3f46]"
                  type="text"
                  maxLength={1}
                  value={digit}
                  onChange={(e) =>
                    handleOtpInput(index, e.target.value, setOtp, otp)
                  }
                  onKeyDown={(e) => handleOtpKeyDown(index, e)}
                  placeholder="-"
                />
            ...
            <p className="text-center text-[10px] text-[#71717a] mt-4">
              Didn't receive code?{" "}
              <button className="text-blue-500 hover:text-blue-400 font-medium hover:underline transition-colors">
                Resend
              </button>
            </p>
          </div>
